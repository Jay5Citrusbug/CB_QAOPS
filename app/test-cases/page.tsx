...
import { Search, ChevronRight, Shield, User, AlertCircle, LayoutGrid, List } from "lucide-react";
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="premium-card !p-0 overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/50">
           ...
           <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-white px-3 py-1.5 rounded-lg border border-slate-100 shadow-sm shrink-0">{filteredProjects.length} projects found</span>
        </div>
                        ...
                        {filteredProjects.map((project) => (
                            <tr key={project.id} className="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td className="px-6 py-4 text-right">
                                    <Link href={`/test-cases/${project.id}`} className="inline-flex items-center gap-1 px-3 py-1.5 bg-slate-50 text-[#ed5c37] hover:bg-[#ed5c37] hover:text-white border border-[#ed5c37]/10 rounded-xl text-xs font-semibold transition-all">
                                        View Cases
